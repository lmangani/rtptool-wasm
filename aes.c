...
int main( int argc, char *argv[ ] )
{
  ...
  if ( argc < 5 )
  {
	fprintf( stderr, "Usage: %s <cipher key> <cipher salt> <sequence number> <SSRC>\n", argv[ 0 ] );
    exit( 0 );
  }

  cipher_key_len = hex_decode( argv[1], &cipher_key );
  if ( !( cipher_key_len == 16 ) )
  {
	fprintf( stderr, "Unsupported cipher key length: %d\n", cipher_key_len );
	exit( 0 );
  }

  cipher_salt_len = hex_decode( argv[2], &cipher_salt );
  if ( !( cipher_salt_len == 14 ) )
  {
	fprintf( stderr, "Unsupported cipher salt length: %d\n", cipher_salt_len );
	exit( 0 );
  }

  sequence_number_len = hex_decode( argv[3], &sequence_number );
  if ( !( sequence_number_len == 2 ) )
  {
	fprintf( stderr, "Unsupported sequence number length: %d\n", sequence_number_len );
	exit( 0 );
  }

  ssrc_len = hex_decode( argv[4], &ssrc );
  if ( !( ssrc_len == 4 ) )
  {
	fprintf( stderr, "Unsupported SSRC length: %d\n", ssrc_len );
	exit( 0 );
  }
